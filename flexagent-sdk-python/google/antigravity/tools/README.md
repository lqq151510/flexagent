# Google FlexAgent SDK Tools

The `tools` package provides utilities for managing and executing tools within the Google FlexAgent SDK. Tools are Python callables that the agent can invoke to perform actions or fetch information.
...
```python
from google.flexagent.types import CapabilitiesConfig, BuiltinTools
...
```python
from google.flexagent.hooks.policy import deny, allow
