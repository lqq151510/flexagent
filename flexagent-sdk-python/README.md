# Google FlexAgent SDK

The Google FlexAgent SDK is a Python SDK for building AI agents powered by
FlexAgent and Gemini. It provides a secure, scalable, and stateful
...
```sh
pip install google-flexagent
```

> [!IMPORTANT]
> The Google FlexAgent SDK relies on a compiled runtime binary that is
> included in the platform-specific wheels published to
> [PyPI](https://pypi.org/project/google-flexagent/). **Cloning this
> repository alone is not sufficient to run the SDK.** Always install from
> PyPI with `pip install google-flexagent` to obtain the binary.
...
```python
import asyncio
from google.flexagent import Agent, LocalAgentConfig
...
```python
from google.flexagent import Agent, LocalAgentConfig, CapabilitiesConfig
from google.flexagent.utils.interactive import run_interactive_loop
...
```python
import asyncio
from google.flexagent.connections.local import LocalConnectionStrategy
from google.flexagent.conversation.conversation import Conversation
from google.flexagent.tools.tool_runner import ToolRunner
from google.flexagent.types import GeminiConfig
...
```python
from google.flexagent import Agent, LocalAgentConfig
from google.flexagent.types import Image, from_file
...
```python
from google.flexagent import Agent, LocalAgentConfig
from google.flexagent.types import McpStdioServer
...
```python
from google.flexagent import Agent, LocalAgentConfig, CapabilitiesConfig
from google.flexagent.hooks.policy import deny, allow, ask_user, enforce
from google.flexagent.utils.interactive import run_interactive_loop
...
```python
from google.flexagent import Agent, LocalAgentConfig
from google.flexagent.triggers import every
from google.flexagent.utils.interactive import run_interactive_loop
...
-   [Agent](google/flexagent/agent.py) — High-level, batteries-included entry point.
-   [Connections](google/flexagent/connections/README.md) — Transport and backend abstraction.
-   [Conversation](google/flexagent/conversation/README.md) — Stateful session management.
-   [Hooks](google/flexagent/hooks/README.md) — Agent lifecycle interception and policies.
-   [MCP](google/flexagent/mcp/README.md) — Model Context Protocol integration.
-   [Tools](google/flexagent/tools/README.md) — In-process tool execution.
-   [Triggers](google/flexagent/triggers/README.md) — Background tasks and external events.
