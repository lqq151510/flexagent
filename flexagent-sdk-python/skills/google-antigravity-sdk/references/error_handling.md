...
# Error Handling and Debugging in Google FlexAgent SDK

This guide is intended for agents assisting users in troubleshooting their
Google FlexAgent agents and making them more robust.
...
*   **`FlexAgentValidationError`**: Raised when input validation fails (e.g., invalid parameters passed to a tool or configuration).
*   **`FlexAgentConnectionError`**: Raised when connection issues occur (e.g., WebSocket drops, timeout).

Example:

```python
from google.flexagent import types

try:
    # Agent operations
    pass
except types.FlexAgentValidationError as e:
    print(f"Validation failed: {e}")
except types.FlexAgentConnectionError as e:
...
from google.flexagent.hooks import hooks
...
```python
from google.flexagent.connections.local import LocalAgentConfig
