---
name: google-flexagent-sdk
description: "Design, implement, and debug autonomous AI agents and multi-agent systems using the Google FlexAgent (AGY) SDK. ACTIVATE this skill when the user wants to create, configure, or orchestrate Google FlexAgent agents."
---
...
Before proceeding with any Google FlexAgent tasks, ensure the environment is
ready:
...
-   **Check Dependencies**: Check if `google-flexagent` is listed in the
    project's dependencies (e.g., `requirements.txt`, `pyproject.toml`).
-   **Install Package**: Ensure the `google-flexagent` Python package is
    installed.
...
-   If the user needs to understand the high-level overview and core concepts of
    the Google FlexAgent SDK (Agent, Conversation, Connection), read
...
-   If the user needs to add, configure, or load agent skills into the Google
    FlexAgent SDK agent, read `examples/getting_started/agent_skills.md`.
