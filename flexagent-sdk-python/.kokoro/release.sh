...
# Release script for the Google FlexAgent SDK.
#
...
if [[ -n "${KOKORO_ARTIFACTS_DIR}" ]]; then
  cd "${KOKORO_ARTIFACTS_DIR}/git/flexagent-sdk-py"
fi
...
if [[ -n "${PUBLISH_PREBUILT_VERSION:-}" ]]; then
  ...
  echo "=== Google FlexAgent SDK Release v${VERSION} ==="
fi
...
if [[ "${PUBLISH:-}" == "true" || -n "${PUBLISH_PREBUILT_VERSION:-}" ]]; then
  ...
  REPO_URL="https://us-python.pkg.dev/oss-exit-gate-prod/google-flexagent--pypi/"
  echo ""
  ...
  EG_GCS_BUCKET="gs://oss-exit-gate-prod-projects-bucket/google-flexagent/pypi/manifests"
  ...
  echo "  Monitor progress at: http://go/spng2?q=PROJECT%3Agoogle-flexagent%2Fpypi"
  echo ""
